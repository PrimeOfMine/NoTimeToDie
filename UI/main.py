  ...
  def __init__(self, img_in, x, y, width, height, img_act, x_act, y_act, action = None):
    ...
    if x + width > mouse[0] > x and y + height > mouse[1] > y: 
      ...
      if click[0] and action != None: 
        time.sleep(.1)
        for i in imgPath:  
          act_handEraser.append(action(i, model_path))
          mainImg[num] = pygame.surfarray.make_surface(act_handEraser[num]) # surface
          ...
          scale_mainImg[num] = pygame.transform.scale(mainImg[num], (750, 750+250))
...
def execute(): 
  ...
  for i in range(0, len(imgPath)): # 경로에 한글이 들어가지 않도록 주의
    highlight_erasedImg.append(kmeans_color_quantization(act_handEraser[i])) # 이미지 
