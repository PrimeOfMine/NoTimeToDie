...
def draw_image(textlist, TLGAN_save_path, CRNN_save_path):
    ...
    for text, i in zip(textlist, range(len(textlist))):
        ...
        if len(text) <= 100:
            img = np.zeros((30, 256, 3), dtype=np.uint8)
            img=cv2.bitwise_not(img)
            img_pil = Image.fromarray(img)
            draw = ImageDraw.Draw(img_pil)
            draw.text((30, 3), text, font=font, fill=(b, g, r))
            ...
            xmin = 30
            xmax = xmin + 17*len(text)
            ymin = 10 + 30*i
            ymax = ymin+20

            bounding_boxes.append([xmin, xmax, ymin, ymax])
        else:
            full_img = np.concatenate([full_img, np.zeros((30, 256, 3))], axis=0)
            full_img=cv2.bitwise_not(full_img)

    # make sure that the images are in the same size
    if len(textlist) < args.n_text:
        full_img = np.concatenate([full_img, np.zeros((30*(args.n_text-len(textlist)), 256, 3),)], axis=0)
        full_img[len(textlist)*30:,:]=[255,255,255]
    ...
    if full_img.shape[0] < 256:
        ...
        full_img[full_img_0.shape[0]:,:]=[255,255,255]
